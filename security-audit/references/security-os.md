...
1. **SSH**: Check /etc/ssh/sshd_config → verify key-only auth, no root login
2. **Sudo**: Check /etc/sudoers → verify minimal privileges, no NOPASSWD for sensitive commands
3. **Updates**: Run apt list --upgradable → verify recent updates
4. **Accounts**: Check /etc/passwd, chage -l → verify password policies, no default accounts
5. **Services**: Run systemctl list-units → verify only necessary services running
6. **Permissions**: Check critical files → verify proper permissions (shadow, ssh keys)
7. **Logging**: Check systemctl status rsyslog auditd → verify logging enabled
8. **Firewall**: Check ufw status / iptables -L → verify restrictive rules
9. **Kernel**: Check sysctl settings → verify hardening enabled
10. **IDS**: Check for AIDE/fail2ban → verify intrusion detection installed

## Report Format

```
**[CRITICAL] SSH Password Authentication Enabled**
- **Category**: OS & Server Security
- **Location**: `/etc/ssh/sshd_config:42`
- **Issue**: PasswordAuthentication is set to yes, allowing password-based SSH login
- **Risk**: Brute force attacks can compromise SSH access; key-based auth is more secure
- **Evidence**: `PasswordAuthentication yes`
- **Fix**: Set `PasswordAuthentication no` and restart SSH: `systemctl restart sshd`
- **Reference**: CWE-287, A07:2021
```

## Common References

CWE-250 (execution with unnecessary privileges) | CWE-269 (improper privilege management) | CWE-287 (improper authentication) | CWE-306 (missing authentication) | CWE-307 (excessive auth attempts) | CWE-521 (weak password requirements) | CWE-668 (exposure of resource) | CWE-693 (protection mechanism failure) | CWE-732 (incorrect permissions) | CWE-778 (insufficient logging) | CWE-1035 (vulnerable component) | CWE-1104 (unmaintained component) | CWE-1188 (insecure default) | A01:2021 (Broken Access Control) | A05:2021 (Security Misconfiguration) | A06:2021 (Vulnerable Components) | A07:2021 (Auth Failures) | A09:2021 (Logging Failures)
