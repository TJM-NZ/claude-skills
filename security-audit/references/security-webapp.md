...
1. **XSS**: Grep innerHTML, dangerouslySetInnerHTML, v-html → verify escaping
2. **CSRF**: Find forms with POST/PUT/DELETE → verify CSRF tokens, SameSite cookies
3. **Headers**: Curl responses → check for security headers (X-Frame-Options, CSP, etc.)
4. **CSP**: Check Content-Security-Policy → verify no unsafe-inline/eval
5. **Clickjacking**: Check X-Frame-Options / frame-ancestors
6. **Cookies**: Find Set-Cookie → verify HttpOnly, Secure, SameSite flags
7. **SRI**: Find CDN script/link tags → verify integrity attributes
8. **HTML injection**: Search innerHTML usage → verify sanitization
9. **Autocomplete**: Find password/sensitive inputs → verify autocomplete off
10. **Referrer**: Check Referrer-Policy header → verify set appropriately
...
- **Location**: `views/search.html:12`
- **Issue**: User search query rendered unescaped in HTML
- **Risk**: Attacker can inject JavaScript, steal session cookies, perform actions as victim
- **Evidence**: `<h1>Results for: {{ query | safe }}</h1>`
- **Fix**: Remove `| safe` filter to auto-escape: `<h1>Results for: {{ query }}</h1>`
- **Reference**: CWE-79, A03:2021
```

## Common References

CWE-79 (XSS) | CWE-200 (information exposure) | CWE-352 (CSRF) | CWE-353 (missing integrity check) | CWE-522 (insufficiently protected credentials) | CWE-1004 (sensitive cookie) | CWE-1021 (improper restriction) | A01:2021 (Broken Access Control) | A03:2021 (Injection) | A05:2021 (Security Misconfiguration) | A08:2021 (Software Integrity Failures)
