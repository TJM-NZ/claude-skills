...
1. **Hardcoded secrets**: Grep for password/api_key/secret → verify no hardcoded values
2. **Git history**: Check `git ls-files` → verify no .env, .key, credentials files
3. **Logs**: Search console.log, logger → verify no passwords/tokens logged
4. **Error handling**: Find catch blocks → verify generic errors to users
5. **PII**: Search for email/phone/ssn → verify minimal exposure, encryption
6. **URLs**: Grep for password=, token= → verify secrets not in URLs
7. **Encryption**: Check data storage → verify encryption for PII/sensitive data
8. **Backups**: Find backup scripts → verify encrypted, secure storage
9. **Debug mode**: Check DEBUG, NODE_ENV → verify disabled in production
10. **Retention**: Check delete logic → verify hard deletes, retention policies
...
- **Location**: `config/stripe.js:5`
- **Issue**: Stripe secret key hardcoded in source code
- **Risk**: Anyone with repo access can steal API key, charge customers, access payment data
- **Evidence**: `const STRIPE_SECRET = "sk_live_abc123xyz789";`
- **Fix**: Move to environment variable: `const STRIPE_SECRET = process.env.STRIPE_SECRET_KEY;`
- **Reference**: CWE-798, A02:2021
```

## Common References

CWE-11 (ASP.NET debug) | CWE-209 (error message info) | CWE-215 (debug info) | CWE-259 (hardcoded password) | CWE-311 (missing encryption) | CWE-312 (cleartext storage) | CWE-359 (privacy violation) | CWE-497 (exposure of system data) | CWE-522 (insufficiently protected credentials) | CWE-532 (log files) | CWE-598 (GET for sensitive data) | CWE-798 (hardcoded credentials) | A02:2021 (Crypto Failures) | A04:2021 (Insecure Design) | GDPR
