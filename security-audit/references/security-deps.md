...
1. **Vulnerabilities**: Run npm audit / pip-audit → check for known CVEs
2. **Outdated**: Run npm outdated → check for very old packages
3. **Lock files**: Verify package-lock.json / yarn.lock committed
4. **Confusion**: Check for unscoped private package names
5. **Integrity**: Search for CDN scripts → verify SRI hashes
6. **Sources**: Check registries → verify HTTPS only
7. **Typos**: Review package.json → verify package names correct
8. **Transitive**: Run npm ls → check full dependency tree
9. **Licenses**: Run license-checker → verify compatible licenses
10. **Automation**: Check for Dependabot / Renovate / Snyk setup
...
- **Issue**: lodash 4.17.19 has prototype pollution vulnerability (CVE-2020-8203)
- **Risk**: Attacker can modify object prototype, leading to DoS or RCE
- **Evidence**: `npm audit` shows 1 high severity vulnerability
- **Fix**: Update to lodash 4.17.21: `npm install lodash@4.17.21`
- **Reference**: CVE-2020-8203, CWE-1035, A06:2021
```

## Common References

CWE-327 (broken crypto) | CWE-353 (missing integrity check) | CWE-427 (uncontrolled search path) | CWE-494 (download without integrity check) | CWE-506 (embedded malicious code) | CWE-829 (untrusted control sphere) | CWE-924 (improper communication channel) | CWE-1035 (vulnerable third-party component) | CWE-1104 (unmaintained third-party component) | CWE-1395 (dependency on vulnerable third-party component) | A06:2021 (Vulnerable Components) | A08:2021 (Software Integrity Failures)
