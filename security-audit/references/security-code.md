...
1. **Race conditions**: Find async operations → check for TOCTOU, non-atomic updates
2. **Business logic**: Review price/discount/balance logic → verify server-side validation
3. **Deserialization**: Search pickle, yaml.load, unserialize → verify safe alternatives
4. **ReDoS**: Find regex patterns → check for nested quantifiers
5. **Integer issues**: Find arithmetic → verify range checks, overflow prevention
6. **Error handling**: Find catch blocks → verify logging, no silent failures
7. **Reflection**: Search require/import with variables → verify whitelisting
8. **Resource limits**: Find loops/allocations → verify size limits, timeouts
9. **TOCTOU**: Find file existence checks → verify atomic operations
10. **Type safety**: Search == operators → verify strict equality, type validation

## Report Format

```
**[HIGH] Race Condition in Balance Update**
- **Category**: Code Security Patterns
- **Location**: `api/transfer.js:23`
- **Issue**: Balance check and update not atomic, allows concurrent transfers
- **Risk**: User can overdraw account by making simultaneous transfer requests
- **Evidence**: `if (balance >= amount) { balance -= amount; save(); }`
- **Fix**: Use database transaction with row-level lock
- **Reference**: CWE-362, A04:2021
```

## Common References

CWE-190 (integer overflow) | CWE-191 (integer underflow) | CWE-362 (race condition) | CWE-367 (TOCTOU) | CWE-390 (error handling) | CWE-400 (resource exhaustion) | CWE-470 (unsafe reflection) | CWE-502 (insecure deserialization) | CWE-755 (improper error handling) | CWE-770 (allocation without limits) | CWE-840 (business logic) | CWE-843 (type confusion) | CWE-1333 (ReDoS) | A03:2021 (Injection) | A04:2021 (Insecure Design) | A08:2021 (Software Integrity)
