...
Read `PRODUCT.md`, `CLAUDE.md`, terminology docs. Load `.claude/docs/tone.md` — if missing, suggest `/copy-tone` first. Check existing email templates and sending logic in the codebase — new emails should match the existing tone and format.
...
No emdashes | no unlock/unleash/elevate/empower/leverage/streamline | no "we're thrilled/excited" | no "Dear valued user" | no ! in subject lines. Contractions fine. Plain words ("new" not "brand-new"). If it reads like a SaaS onboarding template from 2019, rewrite it.
