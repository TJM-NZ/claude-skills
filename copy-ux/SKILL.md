...
Read `PRODUCT.md`, `CLAUDE.md`, terminology docs. Load `.claude/docs/tone.md` — if missing, suggest `/copy-tone` first. Search for similar microcopy patterns (empty states, errors, buttons) in the codebase — new copy should match existing tone.
...
No emdashes | no unlock/unleash/elevate/streamline/empower | no ! in UI. Contractions fine ("can't" not "cannot"). Plain concrete words ("save" not "persist", "show" not "display"). If it sounds like a robot wrote it, rewrite it.
